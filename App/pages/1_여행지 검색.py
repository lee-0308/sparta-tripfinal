...
def get_flag_path(city, country):
    ...
    folder = os.path.join("..", "Images", "Flags")
...
def find_image(city_name, place_name):
    ...
    folder = os.path.join("..", "Images", "Spots")
    for ext in [".jpg", ".jpeg", ".png", ".webp", ".avif"]:
        path = os.path.join(folder, f"{safe_city}_{safe_place}{ext}")
        if os.path.exists(path):
            return path
    return None
...
df_info = pd.read_csv("C:/sparta-tripfinal/Files/worldcities_cost_fin.csv")
df_spot = pd.read_csv("C:/sparta-tripfinal/Files/revie_spots_with_coords.csv")
df_airport = pd.read_csv("C:/sparta-tripfinal/Files/airports_full_done.csv")
df_hotel = pd.read_csv("C:/sparta-tripfinal/Files/hotels_with_lalong.csv")
df_flight = pd.read_csv("C:/sparta-tripfinal/Files/LCC_FSC_Hour_Price_USD.csv")
